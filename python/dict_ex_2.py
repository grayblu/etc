...
result1 = len(mulcam['location'])
print('1번 정답: {}'.format(result1))

result2 = 'request' in mulcam['language']['python']['python standard library']
print('2번 정답: {}'.format(result2))

result3 = mulcam['classes']['서울']['class president']
print('3번 정답: {}'.format(result3))

for lan in mulcam['language'].keys():
    print(lan)

for name in mulcam['classes']['부산'].values():
    print(name)

for name, des in mulcam['language']['python']['frameworks'].items():
    print(f'{name}는 {des}이다.')

import random
dangbun = mulcam['classes']['서울']['groups']['4조']
name = random.choice(dangbun)
print(name)